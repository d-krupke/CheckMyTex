    ...
    def check(self, document: LatexDocument) -> typing.Iterable[Problem]:
        ...
        for match in regex.finditer(text):
            ...
            if self.check_ranges:
                assert text[s_origin[0]:s_origin[1]] == text[match.start():match.end()]
                assert text[t_origin[0]:t_origin[1]] == text[match.start():match.end()]
                assert text[f_origin[0]:f_origin[1]] == text[match.start():match.end()]
            yield Problem(origin, "ABC found.", text[match.start(): match.end()],
                          str(origin), "ABC", "ABC")


class AbcTest(unittest.TestCase):
    def test_1(self):
        files = {"/main.tex": "This is a test with ABC.\nSecond line. Also here an ABC."}
    ...
    def test_2(self):
        ...
        analysis = engine.analyze(document)
        self.assertEqual(document.get_text(), "This is a test with ABC.\nSecond line. Also here an ABC.\nHere one with newcommand ABC.")
        self.assertEqual(len(analysis.problems), 3)
        for problem in analysis.problems:
            f_range = problem.origin.get_file_span()
            print(problem.serialize(), "in file:", files["/main.tex"][f_range[0]:f_range[1]])
