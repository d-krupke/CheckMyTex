...
class LatexDocumentTest(unittest.TestCase):
    def test_1(self):
        ...
        document = LatexDocument("/main.tex",
                        file_finder=FileFinder("/", "/main.tex", sources))
        ...
        for i in range(4):
            ...
            o3 = Origin("/main.tex", Origin.Position(i, 0, i),
                        Origin.Position(i + 1, 0, i + 1))
        ...
        for i in range(5, 8):
            ...
            o1 = document.get_origin_of_source(i, i + 1)
            o2 = document.get_origin_of_text(i, i + 1)
            o3 = Origin("/main.tex", Origin.Position(i, 1, j),
                        Origin.Position(i + 1, 1, j + 1))
            self.assertEqual(o1, o2)
            self.assertEqual(o1, o3)
    ...
    def test_2(self):
        ...
        for i in range(5, 8):
            ...
            o1 = document.get_origin_of_source(i, i + 1)
            o2 = document.get_origin_of_text(i, i + 1)
            o3 = Origin("/sub.tex", Origin.Position(j, 0, j),
                        Origin.Position(j + 1, 0, j + 1))
            self.assertEqual(o1, o2)
            self.assertEqual(o1, o3)
    ...
    def test_3(self):
        ...
        for f in ["A", "B", "C"]:
            for i in range(0, 3):
                key = f"{f}{i}"
                origin = Origin("/" + f + ".tex", Origin.Position(3 * i, i, 0),
                                Origin.Position(3 * i + 1, i, 1))
                p = document.get_text().find(key)
                self.assertEqual(origin,
                                 document.get_origin_of_source(p, p + 1))
